...
import { getVideoEmbedUrl, usesNativeVideoPlayer } from "@/lib/video-embed";
...
export function AdminShortCard({ short }: AdminShortCardProps) {
  const usesNativeVideo = usesNativeVideoPlayer(
    short.video_url,
    short.video_provider,
  );
  const embedUrl = usesNativeVideo
    ? null
    : getVideoEmbedUrl(short.video_url, short.video_provider);
